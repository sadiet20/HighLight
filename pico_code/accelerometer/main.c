...
int main() {
    ...
    while (true) {
        ...
        RMS = sqrt((acc_x*acc_x + acc_y*acc_y + acc_z*acc_z)/3.0);              //unadjusted RMS based on raw data
        RMS_f = sqrt((acc_x_f*acc_x_f + acc_y_f*acc_y_f + acc_z_f*acc_z_f)/3);  //adjusted RMS based on m/s^2

        // Print results
        printf("X: %.2f m/s^2 | Y: %.2f m/s^2 | Z: %.2f m/s^2 | RMS: %.2f \r\n", acc_x_f, acc_y_f, acc_z_f, RMS_f);
